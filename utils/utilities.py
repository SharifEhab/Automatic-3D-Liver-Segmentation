...
def dice_metric(pred, target):
    ...
    value = 1 - dice_loss(pred, target).item()
    return value
...
def train(model, dataloader, loss_function, optimizer,max_epochs, model_dir, test_interval =1, device = torch.device("cuda:0"), logger = None, log_freq = 10):
    """
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            label = label!=0 #creates a binary mask where the liver and its lesions are labeled as 1 and the background is labeled as 0
            ...
            with autocast(device_type=device.type):  # Enables mixed precision computation for this block
                ...
                train_loss = loss_function(outputs.float(), label.float())
           ...
           # print(
            ...
            train_metric = dice_metric(outputs.float(), label.float())
            ...
            with torch.no_grad(): # disable gradient calculation during evaluation to save memory and computation during validation.
                ...
                for test_data in test_loader:
                    ...
                    with autocast(device_type=device.type):  # Mixed precision inference
                        ...
                        test_loss = loss_function(test_outputs.float(), test_label.float())
                    ...
                    test_metric = dice_metric(test_outputs.float(), test_label.float()) # calculate the dice metric for the current test batch
                ...
                if logger:
                    logger.log({
                        "epoch": epoch + 1,
                        "test_loss_epoch": test_epoch_loss,
                        "test_dice_epoch": epoch_metric_test,
                    })
